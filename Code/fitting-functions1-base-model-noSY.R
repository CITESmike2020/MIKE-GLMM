# Model to fit the PIKE data WITHOUT Year.Site effects
...
fit.pike.noSY <- function(pike, mike.pop.est,
...
# Fit a hierarchical Bayesian model to pike data without a Year.Site effect.
# Parametera are
...
# make sure there is no current site or year index variable on the PIKE or population dataset
...
############################################################
  ...
  for(i in 1:Nsiteyear){
     IC[i] ~ dbin(pi[i], TC[i])
     logit(pi[i]) <- Year.eff[ year.index[i] ]+
                     Site.eff[ site.index[i] ]
   }
  ...
  # Compute the estimated logit(PIKE) for each site.year
  for(y in 1:Nyear){
     for(s in 1:Nsite){
       Year.Site.est [y,s] <- Year.eff[y] + Site.eff[s]
  ...
  for(y in 1:Nyear){
     Year.eff.MM [y] <- mean( Year.Site.est [y, 1:Nsite])
     YearP.eff.MM[y] <- mean( Year.SiteP.est[y, 1:Nsite])
  }
 
  # Weighted marginal means on p-scale using the mike;pop weights
  for(y in 1:Nyear){
     YearP.eff.MMweighted[y] <- inprod( Year.SiteP.est[y, 1:Nsite], mike.pop[y, 1:Nsite])/ sum(mike.pop[y, 1:Nsite])
  }  
...
data.list <- list(
  ...
  IC        = pike[, IC.var], 
  TC        = pike[, TC.var],
  year.index= pike$year.index, 
  site.index= pike$site.index,
  mike.pop  = mike.pop   # matrix [year,site] of values
)

# initial value list
init.list <- list(
      list(Year.eff=rep(logit(sum(pike[,IC.var])/sum(pike[,TC.var])),Nyear)),
      list(Year.eff=rep(logit(sum(pike[,IC.var])/sum(pike[,TC.var])),Nyear)),
      list(Year.eff=rep(logit(sum(pike[,IC.var])/sum(pike[,TC.var])),Nyear))
...
monitor.list <- c( "Year.eff","Year.eff2","YearP.eff",
                  "Year.eff.MM","YearP.eff.MM","YearP.eff.MMweighted", # marginal means
                  "Site.eff","sd.site.eff",
                   "Year.Site.est",  # Year-Site estimates and marginal means
