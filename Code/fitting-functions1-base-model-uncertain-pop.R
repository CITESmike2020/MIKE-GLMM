...
# This differs from the base model because we allow for a SD of 25% in the abundance numbers

fit.pike.SDW25 <- function(pike, mike.pop.est,
...
# Fit a hierarchical Bayesian model to pike data bu
# Parametera are
...
############################################################
  ...
  # allow for 25% uncertainty in the MIKE population estimates
  # This is not fully accurate because poplation estimates are not found every year
  # and error in estimates are not likely to be independent over time.
  for(y in 1:Nyear){
    for(j in 1:Nsite){
       sd.mike.pop[y,j] <- max(mike.pop[y,j]*0.25,2) # avoid an SD of 0!
       mike.pop2[y,j] ~ dnorm(mike.pop[y,j], 1/(sd.mike.pop[y,j]^2));T(0,)  # need semi colon see https://sourceforge.net/p/mcmc-jags/discussion/610037/thread/3eaa3bbb/?limit=25
    }
  }
  
  # Weighted marginal means on p-scale using the mike pop weights
  for(y in 1:Nyear){
     YearP.est.MMw[y] <- inprod( Year.SiteP.est[y, 1:Nsite], mike.pop2[y, 1:Nsite])/ sum(mike.pop2[y, 1:Nsite])
  }  
  ...
  for(y in 1:Nyear){
     YearP.est.MMw.boot[y] <- inprod( Site.select[1:Nsite], 
                                      Year.SiteP.est[y, 1:Nsite]* mike.pop2[y, 1:Nsite]/
                                        inprod(Site.select[1:Nsite], mike.pop2[y, 1:Nsite]))
  } 
} # End of the model
