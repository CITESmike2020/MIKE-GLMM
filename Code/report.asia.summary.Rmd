---
title: "MIKE analysis for Asia - Summary"
author: "Carl Schwarz"
...
subregion and year as factors. **There is no subregional *LSMeans* model for *PIKE* for Asia.**
...
in each site at the sub-regional level.
For example, in South Asia, both India (10 MIKE sites) and Bangladesh (1 MIKE site)
are given equal weight.
But at the continental level, countries are ignored and data pooled over all sites in a sub-region.
- It is difficult to apply different weighting, e.g. by elephant populations at the *MIKE* or country level.
- The *LSMeans* approach ignores that fact the individual *MIKE* sites could differ in the 
patterns of missingness across years. Consequently, aggregate data is based on a different
combination of *MIKE* sites across the years.
...
The impact of giving equal weight at the country level or at the *MIKE* site level is explored in more detail in Appendix 3 of 
the Africa technical document
...
abundance estimates (see Section 8.7 of the technical document for Africa).
...
MIKE sites in Asia broken into 
`r length(unique(pike$SubregionName))` subregions. 
...
**Notice that no data has been reported from South Asia past 2017** and so the *LSMeans* are only available
to 2017. The GLMM approach will impute values for the missing years just like other missing years are imputed.
...
Note that only a very small number of carcasses are reported for MIKE sites in South East Asia.
...
plotset <- arrangeGrob(grobs=plotlist, nrow=2, ncol=1, widths=c(5),  heights=c(5,5),
...
In Africa sites, patrol effort varies considerably among sites, so the number of examined carcasses only bears
a weak relationship to estimated population abundances around the MIKE sites. **However, we currently do
not have estimates of elephant abundance for all of the Asia sites, so cannot fully explore this relationship.**
...
# we currrently use population size of 1 to represent missing
plotdata$population[ plotdata$population==1] <- NA
...
There appears to be a weak relationship in South Asia, but no relationship in South East Asia.
...
**We currently do not have elephant abundance data for all Asia MIKE sites and for sites with population
data, we only have have a single value which will be used for all years in the study.**.

<!--- Population data is available for `r sum(data.source$Population)` sites and is extracted
from XXXXXXXXXXXXXXXXX (XXXXX)
...
# we want to add back the MIKEsiteID for cases with all missing data
temp <- mike.pop.est.original
temp$pop.cat[ temp$population < 5] <- NA

pop.plot <- ggplot(data=temp, aes(x=year, y=MIKEsiteName, color=pop.cat))+
...
The final dataset <!--- is an amalgamation of the MIKE population data and ---> 
consists of the observed *PIKE* data without any population data (because not all site have
population data). Only those sites
<!---for which population data is available and --->
for which at least one carcass has been reported over the period
...
kable(temp[,c("MIKEsiteID","Population","pike","GIS")], row.names=FALSE,
      caption="Summary of MIKE sites where no PIKE data available",
...
unweighted marginal mean *PIKE* is found as the simple average of the *PIKE* in a year over all sites.
...
**This section is more relevant to the Africa analysis because population data
is not availble for all MIKE sites in Asia. It has been retained to maintain consistency between
the summary documents for Asia and Africa pending obtaining population data.**
...
The estimated *LSMeans* and unweighted <!---and weighted--->
marginal mean  *PIKE* are:
...
temp <- merge(temp, plyr::rename(lsmeans.cont[,c("year","mean","se")], c("year"="Year")))
temp <- temp[ order(temp$Year),]

kable(temp[, c(1:3,6:7)], 
      caption="Estimated marginal mean PIKE from the GLMM and the LSMeans",
      col.names=c("Year","Mean","SD","Mean","SD"),
      digits=c(0,2,2,2,2))  %>% 
      add_header_above(c(" "=1, "Unweighted "=2, "LSMeans "=2)) %>%
...
and the the unweighted marginal mean (*MM.p.uw*) <!--- and weighted 
marginal
...
ggplot(data=plotdata, aes(x=year, y=mean, color=Source))+
  ggtitle(paste(UNRegion.select,": Estimated PIKE across time",sep=""),
          subtitle="Comparison of observed, LSMeans and unweighted marginal mean PIKE")+
...
fairly well, except for 2018 and 2019 where no data was received from one subregion.
The fitted trend lines are consistently higher than the
...
<!--- The weighted marginal mean ($MM.p.w$) tracks the observed *PIKE* closely after 2010 because
...
of fitted slopes that are less than zero. 
...
The posterior belief that the slope in PIKE
...
**The current LSMeans approach does not report estimates at the subregional level,
but we modified the current approach to compute sub-regional estimates based on a similar model
used in Africa that includes a country and a year effect.
Consequently, these results are only preliminary**

The above GLMM analyses were repeated at the sub-regional level. Only the data from each sub-region was used in
each analysis, i.e., completely separate analyses were performed for each sub-region. 

A more complex model where
the 
`r length(unique(pike$SubregionName))` sub-regions were modelled as part of a larger model could be attempted.
...
This more complex model is examined in the Asia technical document, but because there does not appear to be 
...
The following plots compare the mean *PIKE* computed using the $LSMeans$ method and 
the unweighted marginal mean *PIKE*  (*MM.p.uw*). <!--- and the weighted marginal mean *PIKE* (*MM.p.w*).--->

```{r echo=FALSE, warning=FALSE, message=FALSE, results="hide"}
#, out.height="450px", out.width="600px"}
...
compared to the continental results. This is especially true for the South East Asia subregion where
the average number of carcasses monitored is very small.

<!--- It is hard to make a general statement comparing the estimates computed using $LSMeans$ and the
unweighted marginal means (*MM.p.uw*) because of the wide confidence intervals, but --->
The respective trend
lines mostly track each other.
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
#, out.height="600px", out.width="600px"}
...
We see that *PIKE* in Southern Asia is consistently lower than the continental
*PIKE*, while *PIKE* in South East Asia is consistently higher. 
...
* Site *NIL*. This site reports nearly every year with a large number of carcasses (large blue circles). The estimated
yearly site-level *PIKE* closely follows the observed data (as expected).

* Site *CHR*. In some years, this site reports a large number of carcasses and the estimated yearly site-level *PIKE*
for these years matches the observed *PIKE*. In some years, the observed *PIKE* based on large number of carcasses is
above the continental marginal mean (e.g. 2007) and in some some years it is below the continental marginal mean (e.g. 2012.
...
* Site *CHU*. This site mostly has smallish sample sizes, but the observed *PIKE* is consistently close to 0.
...
site.effects.spatial.plot2 <- base.map +
...
unless the new site has an extreme *PIKE*.
<!--- The weighted marginal mean *PIKE* could be influenced
...
extended to finding weighted marginal mean *PIKE* (e.g. weighted by population abundances when these become available)
